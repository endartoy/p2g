...
function tesApp() {
    ...
    return {
        ...
        init() {
            if (Alpine.store('page') !== 'tes app') return;
            ...
            if (this.subNotifItem) {
                this.subNotifItem();
            }

            this.subNotifItem = db.collection('_sinoman').where('chanel', '==', this.newItem.chanel).onSnapshot(snapshot => {
                if (isFirstLoad) {
                    isFirstLoad = false;
                    return;
                }
                ...
                snapshot.docChanges().forEach(change => {
                    ...
                    if (i.chanel === this.newItem.chanel) {
        ...
        resetForm() {
            ...
            this.newItem.chanel = localStorage.getItem('stored_chanel') || '3';
            this.tamu = null;
            this.kukut = false;
            this.form = false;
        },
        ...
        readItem() {
            ...
            this.subReadItem = db.collection('_sinoman').where('chanel', '==', this.newItem.chanel).orderBy('createdAt', 'asc').onSnapshot((snapshot) => {
